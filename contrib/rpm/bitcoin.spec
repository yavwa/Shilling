...
%if %{_buildqt}
# qt icons
install -D -p share/pixmaps/shillingcoin.ico %{buildroot}%{_datadir}/pixmaps/shillingcoin.ico
...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp:34622
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 34621
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 33812
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 33813
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 33834
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 33833
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp:34622
	%{_sbindir}/semanage port -d -p tcp 34621
	%{_sbindir}/semanage port -d -p tcp 33812
	%{_sbindir}/semanage port -d -p tcp 33813
	%{_sbindir}/semanage port -d -p tcp 33834
	%{_sbindir}/semanage port -d -p tcp 33833
	for selinuxvariant in %{selinux_variants}; do
