...
def main():
    ...
    tmpdir = "%s/shillingcoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and shillingcoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "shillingcoind"]) is not None:
            print("%sWARNING!%s There is already a shillingcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "SHILLINGCOIND" not in os.environ:
        os.environ["SHILLINGCOIND"] = build_dir + '/src/shillingcoind' + exeext
        os.environ["SHILLINGCOINCLI"] = build_dir + '/src/shillingcoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `shillingcoin-cli help` (`rpc_interface.txt`).
