...
const char * const BITCOIN_CONF_FILENAME = "shillingcoin.conf";
const char * const BITCOIN_PID_FILENAME = "shillingcoind.pid";
...
#else
    const char* pszModule = "shillingcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Shillingcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Shillingcoin";
#else
    // Unix
    return pathRet / ".shillingcoin";
#endif
#endif
}
