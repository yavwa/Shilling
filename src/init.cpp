...
void Shutdown()
{
    ...
    RenameThread("shillingcoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/yavwa/Shilling>";
    const std::string URL_WEBSITE = "<https://digitalshilling.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("shillingcoin-loadblk");
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if shillingcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if shillingcoin is started while in a temporary directory.\n",
            ...
            try {
                ...
                if (!is_coinsview_empty) {
...

                    {
                        ...
                        if (tip && tip->nTime > GetAdjustedTime() + 30 * 60) {
