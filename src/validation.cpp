...
#if defined(NDEBUG)
# error "Shillingcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Shillingcoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s SH additional fees, %d delta bytes\n",
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    if (!CheckProofOfWork(block.GetPoWHash(nHeight >= Params().SwitchLyra2REv2_DGW()), block.nBits, consensusParams))
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // Subsidy is cut in half every 300,000 blocks ..
    nSubsidy >>= halvings;
    return nSubsidy;
}
...
void ThreadScriptCheck() {
    RenameThread("shillingcoin-scriptch");
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetPoWHash(nHeight >= Params().SwitchLyra2REv2_DGW()), block.nBits, consensusParams))
